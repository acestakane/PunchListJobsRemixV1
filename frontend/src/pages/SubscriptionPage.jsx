...
import { CheckCircle, Clock, Zap, Calendar, CreditCard, Shield, ChevronRight } from "lucide-react";
...
const PLAN_ICONS = { daily: Clock, weekly: Calendar, monthly: Zap };
const PLAN_COLORS = { daily: "#10B981", weekly: "#0000FF", monthly: "#7C3AED" };
...
export default function SubscriptionPage() {
  ...
  useEffect(() => {
    ...
    if (methodParam === "paypal" && planParam) {
      ...
      if (orderId) {
        capturePayPal(orderId, planParam);
      }
    }
  }, [searchParams]);
  ...
  const initiateStripe = async () => {
    ...
    try {
      ...
      const res = await axios.post(`${API}/payments/stripe/create-session`, {
        plan: selected, payment_method: "stripe", origin_url: origin
      });
  ...
  const createPayPalOrder = async () => {
    ...
    const res = await axios.post(`${API}/payments/paypal/create-order`, {
      plan: selected, payment_method: "paypal", origin_url: origin
    });
      ...
      <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
          ...
          <div className={`rounded-xl p-4 mb-6 flex items-center gap-3 ${
            ...
            <div>
              <p className="font-semibold text-sm">
                ...
                {subStatus.status === "expired" && "Your subscription has expired"}
              </p>
              {subStatus.subscription_end && (
                <p className="text-xs text-slate-500">{subStatus.status === "active" ? "Renews" : "Expires"}: {new Date(subStatus.subscription_end).toLocaleDateString()}</p>
              )}
            </div>
          </div>
        )}
          ...
          {Object.entries(plans).map(([key, plan]) => {
            ...
            return (
              <button
                ...
                className={`relative p-5 rounded-2xl border-2 text-left transition-all ${isSelected ? "border-[#0000FF] bg-blue-50 dark:bg-blue-950 shadow-lg" : "border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-900 hover:border-slate-300"}`}
                data-testid={`plan-${key}`}
              >
                {key === "monthly" && (
                  <span className="absolute -top-3 left-1/2 -translate-x-1/2 bg-[#0000FF] text-white text-xs font-bold px-3 py-1 rounded-full">Most Popular</span>
                )}
                <div className="w-10 h-10 rounded-xl flex items-center justify-center mb-3" style={{ background: `${color}20` }}>
                  <Icon className="w-5 h-5" style={{ color }} />
                </div>
                <div className="font-extrabold text-[#050A30] dark:text-white capitalize text-lg" style={{ fontFamily: "Manrope, sans-serif" }}>{key}</div>
                <div className="text-2xl font-extrabold mt-1" style={{ color }}>
                  ${plan.amount}
                  <span className="text-sm font-normal text-slate-500">/{plan.days === 1 ? "day" : plan.days === 7 ? "week" : "mo"}</span>
                </div>
                <div className="text-xs text-slate-500 mt-1">{plan.label}</div>
                {isSelected && <CheckCircle className="absolute top-4 right-4 w-5 h-5 text-[#0000FF]" />}
              </button>
            );
          })}
        </div>
        ...
        <div className="card p-6 max-w-lg mx-auto">
          <h3 className="font-bold text-[#050A30] dark:text-white mb-4" style={{ fontFamily: "Manrope, sans-serif" }}>
            Pay {selectedPlan ? `$${selectedPlan.amount}` : ""} for {selected} access
          </h3>

          <div className="flex gap-2 mb-6">
            <button
              onClick={() => setMethod("paypal")}
              className={`flex-1 py-2.5 rounded-lg font-bold text-sm border-2 transition-colors ${method === "paypal" ? "border-[#0070BA] bg-[#0070BA]/10 text-[#003087]" : "border-slate-200 dark:border-slate-700 text-slate-500"}`}
            ...
            <button
              ...
              className={`flex-1 py-2.5 rounded-lg font-bold text-sm border-2 transition-colors ${method === "stripe" ? "border-[#635BFF] bg-[#635BFF]/10 text-[#635BFF]" : "border-slate-200 dark:border-slate-700 text-slate-500"}`}
          ...
          {method === "paypal" && (
            <PayPalScriptProvider options={{ "client-id": PAYPAL_CLIENT_ID, currency: "USD" }}>
              <PayPalButtons
                ...
                onError={(err) => { console.error(err); toast.error("PayPal error. Please try again."); }}
